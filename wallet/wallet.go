...
func NewWallet() *Wallet {
	...
	h3.Write(digest2)
	digest3 := h3.Sum(nil)
	// 4. Add version byte in fromt of RiPEMD-160 result,
	vd4 := make([]byte, 21)
	vd4[0] = 0x00
	copy(vd4[1:], digest3[:])
	...
	h6.Write(digest5)
	digest6 := h6.Sum(nil)
	// 7. Take the first 4 bytes of the second SHA-256 hash of checksum.
	chsum := digest6[:4]
...
func (t *Transaciton) GenerateSignature() *utils.Signature {
	m, _ := json.Marshal(t)
...
type TransactoinRequest struct {
...
func (tr *TransactoinRequest) Validate() bool {
