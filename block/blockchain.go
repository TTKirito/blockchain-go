...
const (
	...
	BLOCKAIN_NEIGHBOR_SYNC_TIME_SEC = 20
)
...
func (bc *Blockchain) SetNeighbors() {
	bc.neighbors = utils.FindNeighbors(
		...
		NEIGHBOR_IP_RANGE_START, NEIGHBOR_IP_RANGE_END, BLOCKCHAIN_PORT_RANGE_START, BLOCKCHAIN_PORT_RANGE_END,
	)
...
func (bc *Blockchain) StartSyncNeighbors() {
	bc.SyncNeighbors()
	_ = time.AfterFunc(time.Second*BLOCKAIN_NEIGHBOR_SYNC_TIME_SEC, bc.StartSyncNeighbors)
	return
}
...
func (bc *Blockchain) CreateBlock(nonce int, previousHash [32]byte) *Block {
	...
	for _, n := range bc.neighbors {
		endpoint := fmt.Sprintf("http://%s/transaactions", n)
		client := &http.Client{}
